...
public:
    ...
    int manage_ready()
    {
        ...
        if (this.m.state == MutexState::locked)
        {
            fprintf(stderr, "ERROR: Mutex is LOCKED");
        }
        else
        {
            if (this.m.running_thread_id != nullptr)
            {
                //todo: find running thread and terminate him.
            }

            int running_id = nullptr;
            if(this.m.blocked_list > 0)
            {
                //todo: get blocked id
            }
            else
            {
                ...
                {
                    running_id = this->ready_threads.pop().id;
                }
            }

            this.m.running_thread_id = running_id;
        }
    }
